...
Tests that the server responds correctly when using _include="ExplanationOfBenefit:provider" as a search parameter 
      ...
      run do
        run_search_test('ExplanationOfBenefit:provider', include_search: true, resource_id: c4bb_v200devnonfinancial_explanation_of_benefit_professional_non_clinician_non_financial__id_search_test_param)
      end
    end
  end
end
