...
module CarinForBlueButtonTestKit
  module CARIN4BBV200DEVNONFINANCIAL
    class ExplanationOfBenefitOutpatientInstitutionalNonFinancialExplanationOfBenefit_careteamSearchTest < Inferno::Test
...
Tests that the server responds correctly when using _include="ExplanationOfBenefit:care-team" as a search parameter
      ...
      id :c4bb_v200devnonfinancial_explanation_of_benefit_outpatient_institutional_non_financial_include_ExplanationOfBenefit_careteam_search_test
      ...
      def explanation_of_benefit_outpatient_institutional_non_financial_ids_string_formatter
        return [nil] unless respond_to? :explanation_of_benefit_outpatient_institutional_non_financial_ids
        explanation_of_benefit_outpatient_institutional_non_financial_ids.split(',').map(&:strip).join(',')
      end

      run do
        run_search_test('ExplanationOfBenefit:care-team', include_search: true, resource_id: explanation_of_benefit_outpatient_institutional_non_financial_ids_string_formatter)
      end
    end
  end
end
