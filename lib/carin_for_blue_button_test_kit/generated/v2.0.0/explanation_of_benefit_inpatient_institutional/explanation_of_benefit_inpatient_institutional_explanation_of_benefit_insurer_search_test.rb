...
Tests that the server responds correctly when using _include="ExplanationOfBenefit:insurer" as a search parameter 
      ...
      run do
        run_search_test('ExplanationOfBenefit:insurer', include_search: true, resource_id: c4bb_v200_explanation_of_benefit_inpatient_institutional__id_search_test_param)
      end
    end
  end
end
