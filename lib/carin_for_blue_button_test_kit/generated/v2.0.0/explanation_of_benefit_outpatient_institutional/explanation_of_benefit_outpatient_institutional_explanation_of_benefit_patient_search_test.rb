...
Tests that the server responds correctly when using _include="ExplanationOfBenefit:patient" as a search parameter 
      ...
      run do
        run_search_test('ExplanationOfBenefit:patient', include_search: true, resource_id: c4bb_v200_explanation_of_benefit_outpatient_institutional__id_search_test_param)
      end
    end
  end
end
